...
class SalaryController extends VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'employees', 'salary'));
    }
    ...
    public function salaryOnchange(Request $request){
        $from_date = $request->id1;
        $to_date = $request->id2;
        $employee = $request->id3;

        $data = array('from_date' => $from_date, 'to_date' => $to_date, 'employee' => $employee);
        
        // Get face id
        $face_id_query = Employee::select('face_id')->where('employee_id', $employee)->first();
        $face_id = $face_id_query->face_id;

        // Get total absence
        $absence = Attendance::where('is_present', 0)->whereBetween('attend_date', [$from_date, $to_date])->where('face_id', $face_id)->count();
        
        // Get total late
        $late = Attendance::where('is_late', 1)->whereBetween('attend_date', [$from_date, $to_date])->where('face_id', $face_id)->count();
        
        // Get total deduction from late
        $deduction = Attendance::whereBetween('attend_date', [$from_date, $to_date])->where('face_id', $face_id)->get();
        $total_deduction_late = 0;
        foreach($deduction as $d){
            $total_deduction_late = $d->deduction + $total_deduction_late;
        }
        
        // Get bonus
        $bonus_query = OfficeRule::select('bonus')->first();

        // Get total deduction from absence more than 3 times and set bonus
        if($absence > 3){
            $temp = $absence - 3;
            $total_deduction_absence = $temp * 100000;
            $bonus = 0; 
        }
        else{
            $total_deduction_absence = 0;
            $bonus = $bonus_query->bonus;
        }

        // Count total deduction from late and absence
        $total_deduction = $total_deduction_late + $total_deduction_absence;

        // Get salary
        $salary_query = Employee::select('base_salary')->where('employee_id', $employee)->first();
        $salary = $salary_query->base_salary;
        $salary = $salary - $total_deduction + $bonus;

        echo json_encode(array($absence, $late, $total_deduction, $bonus, $salary));
    }
}
